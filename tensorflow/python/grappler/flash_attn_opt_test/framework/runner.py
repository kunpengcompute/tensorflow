    ...
    def check_fn_default(A, B, meta):
        ...
        for i in range(len(A)):
            if (A[i].dtype == tf.float32):
                is_check_OK &= np.testing.assert_allclose(A[i], B[i], rtol=1e-4, atol=1e-5) == None
            else:
...
class UniversalOpBenchmark:
    ...
    def extract_op_dur(self, timeline_file, op_name, times=1):
        ...
        for trace_events in trace_events_list:
            def is_real_op_event(e, opname):
                args = e.get("args", {})
                return e.get("name") == opname and "input0" in args
            start_event = next(e for e in trace_events["traceEvents"] if is_real_op_event(e, op_name))
            end_event = None
            for e in trace_events["traceEvents"]:
                if e.get("name") == op_name:
                    if "input0" in e.get("args", {}):
                        continue
                    else:
                        end_event = e
            ...
            durations_list.append(total_time)
        if len(durations_list) != times:
            raise ValueError(f"Expected {times} durations for {op_name}, but got {len(durations_list)}")
    ...
    def extract_op_total_time(self, timeline_file, start_op, end_op, times):
        ...
        time_list = []
        for trace_events in trace_events_list:
            def is_real_op_event(e, op_name):
                args = e.get("args", {})
                return args.get("name") == op_name and "input0" in args
            start_event = next(e for e in trace_events["traceEvents"] if is_real_op_event(e, start_op))
            end_event = None
            for e in trace_events["traceEvents"]:
                if e.get("name") == "BatchMatMulV2":
                    if e.get("args", {}).get("name") == end_op:
                        if "input0" in e.get("args", {}):
                            continue
                        else:
                            end_event = e
    ...
    def run_performance_test(self, test_case: TestCase):
        ...
        real_gain = no_fused_time/fused_time
        ...
        print(f"性能测试通过, 性能提升{real_gain:.2f}x, embedding算子融合开启状态下耗时{fused_time:.2f}us, 关闭状态下耗时{no_fused_time:.2f}us")
        return True
    ...
    def run_function_test(self, test_case: TestCase):
        ...
        if not is_correct:
            ...
            return False
        print(f"功能测试通过，算子是否融合：{is_fused}")
        return True
