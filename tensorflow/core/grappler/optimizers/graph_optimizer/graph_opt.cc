...
DECLARE_bool(annc_fused_direct_hash_mod);
DECLARE_bool(annc_fused_dynamic_padding);
DECLARE_bool(annc_fused_trunc_seq);
...
class KPFusedSparseSegmentReduceRewriter : public PatternRewriter {
 public:
  ...
  bool match_and_rewrite(
    ...
    // CHECK_NODE_OK(IsCast(*cast) && cast->input_size() == 1)
    const NodeDef* strided_slice = get_node(ss_reduce->input(2));
...
class KPFusedDirectHashModRewriter : public PatternRewriter {
 public:
  std::string name() const override { return "KPFusedDirectHashMod"; }
  ...
  bool match_and_rewrite(
    ...
    NodeDef* floor_mod = get_operand(node, "FloorMod");
    NodeDef* cast = get_operand(node, "Cast");
    CHECK_NODE_OK(floor_mod != nullptr && cast != nullptr);
    NodeDef* add = get_operand(floor_mod, "AddV2");
    NodeDef* mod_val = get_operand(floor_mod, "Const");
    NodeDef* not_equal = get_operand(cast, "NotEqual");
    CHECK_NODE_OK(add != nullptr && not_equal != nullptr && mod_val != nullptr);
    CHECK_NODE_OK(not_equal->input_size() == 2)
    int not_equal_i = 0;
    NodeDef* not_equal_v = get_mutable_node(not_equal->input(0));
    if (IsConstant(*not_equal_v) || IsHostConstant(*not_equal_v))
      not_equal_i = 1;
    NodeDef* cast_1 = get_operand(add, "Cast");
    NodeDef* add_val = get_operand(add, "Const");
    CHECK_NODE_OK(add_val != nullptr);
    if (cast_1 != nullptr) {
      int cast_1_i = 0;
      NodeDef* cast_1_v = get_mutable_node(cast_1->input(0));
      if (IsConstant(*cast_1_v) || IsHostConstant(*cast_1_v)) cast_1_i = 1;
      CHECK_NODE_OK(get_mutable_node(cast_1->input(cast_1_i)) ==
                    get_mutable_node(not_equal->input(not_equal_i)))
    } else {
      int add_i = 0;
      NodeDef* add_v = get_mutable_node(add->input(0));
      if (IsConstant(*add_v) || IsHostConstant(*add_v)) add_i = 1;
      CHECK_NODE_OK(get_mutable_node(add->input(add_i)) ==
                    get_mutable_node(not_equal->input(not_equal_i)))
    }
    CHECK_NODE_OK(HasNodeAttr(*not_equal, "T"))
    ...
    fused_node->set_op(name());
    fused_node->set_device(node->device());
    fused_node->add_input(not_equal->input(not_equal_i));
    fused_node->add_input(add_val->name());
    fused_node->add_input(mod_val->name());
    AddNodeAttr("T", not_equal->attr().at("T"), fused_node);
    nodes->SwapElements(node_indexes.at(node->name()), nodes->size() - 1);
...
class KPFusedDynamicPaddingRewriter : public PatternRewriter {
 public:
  std::string name() const override { return "KPFusedDynamicPadding"; }
  ...
  bool match_and_rewrite(
    ...
    graph_ = graph;
    indexes_ = &node_indexes;
    CHECK_NODE_OK(IsMerge(*node) && node->input_size() == 2)
    NodeDef* pad = get_operand(node, "Pad");
    NodeDef* switch_3 = get_operand(node, "Switch");
    CHECK_NODE_OK(pad != nullptr && switch_3 != nullptr)
    NodeDef* tsu = get_operand(pad, "TensorScatterUpdate");
    NodeDef* switch_2 = get_operand(pad, "Switch");
    CHECK_NODE_OK(tsu != nullptr && tsu->input_size() == 3)
    NodeDef* fill = get_mutable_node(tsu->input(0));
    NodeDef* reshape_1 = get_mutable_node(tsu->input(1));
    NodeDef* reshape_2 = get_mutable_node(tsu->input(2));
    AttrValue ranked;
    if (IsFill(*fill) && IsReshape(*reshape_1)) {
      CHECK_NODE_OK(fill->input_size() == 2)
      NodeDef* pack = get_mutable_node(fill->input(0));
      NodeDef* fill_const = get_mutable_node(fill->input(1));
      CHECK_NODE_OK(IsPack(*pack) && pack->input_size() == 2 &&
                    check_const_value<int>(fill_const, {0}))
      NodeDef* rank = get_mutable_node(pack->input(0));
      CHECK_NODE_OK(IsRank(*rank) && rank->input_size() == 1 &&
                    get_mutable_node(rank->input(0)) == switch_2)
      NodeDef* pack_const = get_mutable_node(pack->input(1));
      CHECK_NODE_OK(IsReshape(*reshape_1) && reshape_1->input_size() == 2 &&
                    check_const_value<int>(pack_const, {2}))
      NodeDef* sub = get_mutable_node(reshape_1->input(0));
      NodeDef* shape_1_const = get_mutable_node(reshape_1->input(1));
      CHECK_NODE_OK(IsSub(*sub) && sub->input_size() == 2 &&
                    check_const_value<int>(shape_1_const, {1, 1}))
      CHECK_NODE_OK(get_mutable_node(sub->input(0)) == rank)
      NodeDef* sub_const = get_mutable_node(sub->input(1));
      CHECK_NODE_OK(check_const_value<int>(sub_const, {1}))
      ranked.set_i(1);
    } else if (IsConstant(*fill) && IsConstant(*reshape_1)) {
      ranked.set_i(0);
    } else {
      return false;
    }
    CHECK_NODE_OK(IsReshape(*reshape_2) && reshape_2->input_size() == 2)
    NodeDef* pack_2 = get_mutable_node(reshape_2->input(0));
    NodeDef* shape_2_const = get_mutable_node(reshape_2->input(1));
    CHECK_NODE_OK(check_const_value<int>(shape_2_const, {1, 2}))
    CHECK_NODE_OK(IsPack(*pack_2) && pack_2->input_size() == 2)
    NodeDef* pack_2_const = get_mutable_node(pack_2->input(0));
    CHECK_NODE_OK(check_const_value<int>(pack_2_const, {0}))
    NodeDef* sub_2 = get_mutable_node(pack_2->input(1));
    CHECK_NODE_OK(IsSub(*sub_2) && sub_2->input_size() == 2)
    NodeDef* sub_2_const = get_mutable_node(sub_2->input(0));
    sub_2_const->mutable_input()->DeleteSubrange(0, 1);
    NodeDef* switch_1 = get_mutable_node(sub_2->input(1));
    CHECK_NODE_OK(switch_1->op() == "Switch" && switch_1->input_size() == 2)
    NodeDef* less = get_mutable_node(switch_1->input(1));
    CHECK_NODE_OK(less->op() == "Less" && less->input_size() == 2 &&
                  less == get_operand(switch_2, "Less") &&
                  less == get_operand(switch_3, "Less"))
    NodeDef* switch_0{nullptr};
    for (int i = 0; i < graph_->node_size(); ++i) {
      NodeDef* n = graph_->mutable_node(i);
      for (int j = 0; j < n->input_size(); ++j) {
        if (n->input(j) == less->name() && n->op() == "Switch" &&
            n != switch_1 && n != switch_2 && n != switch_3) {
          switch_0 = n;
        }
      }
      if (switch_0 != nullptr) break;
    }
    CHECK_NODE_OK(switch_0 != nullptr)
    NodeDef* strided_slice = get_mutable_node(less->input(0));
    NodeDef* less_const = get_mutable_node(less->input(1));
    CHECK_NODE_OK(strided_slice->op() == "StridedSlice" &&
                  strided_slice->input_size() == 4)
    CHECK_NODE_OK(check_int_attr(strided_slice, "shrink_axis_mask", {1}))
    NodeDef* shape = get_mutable_node(strided_slice->input(0));
    CHECK_NODE_OK(
        check_const_value<int>(get_mutable_node(strided_slice->input(1)),
                               {-1}) &&
        check_const_value<int>(get_mutable_node(strided_slice->input(2)),
                               {0}) &&
        check_const_value<int>(get_mutable_node(strided_slice->input(3)), {1}))
    CHECK_NODE_OK(IsShape(*shape) && shape->input_size() == 1)
    NodeDef* shape_input = get_mutable_node(shape->input(0));
    CHECK_NODE_OK(switch_2->input_size() == 2)
    CHECK_NODE_OK(switch_3->input_size() == 2 &&
                  get_mutable_node(switch_2->input(1)) ==
                      get_mutable_node(switch_3->input(1)))
    CHECK_NODE_OK(shape_input == get_mutable_node(switch_2->input(0)) &&
                  shape_input == get_mutable_node(switch_3->input(0)))
    CHECK_NODE_OK(HasNodeAttr(*shape, "T"))
    ...
    fused_node->set_device(node->device());
    fused_node->add_input(shape->input(0));
    fused_node->add_input(sub_2_const->name());
    fused_node->add_input(less_const->name());
    AddNodeAttr("rank", ranked, fused_node);
    AddNodeAttr("T", shape->attr().at("T"), fused_node);
    ...
    replace_all_users_with(less, 0, fused_node, 0, graph);
    replace_all_users_with(node, 0, fused_node, 1, graph);
    return true;
  }
};

class KPFusedTruncSeqRewriter : public PatternRewriter {
 public:
  std::string name() const override { return "KPFusedTruncSeq"; }
  ...
  bool match_and_rewrite(
    ...
    graph_ = graph;
    indexes_ = &node_indexes;
    CHECK_NODE_OK(node->op() == "StridedSlice" && node->input_size() == 4)
    CHECK_NODE_OK(check_int_attr(node, "begin_mask", 2))
    CHECK_NODE_OK(check_int_attr(node, "ellipsis_mask", 1))
    CHECK_NODE_OK(
        check_const_value<int>(get_mutable_node(node->input(1)), {0, 0}))
    CHECK_NODE_OK(
        check_const_value<int>(get_mutable_node(node->input(3)), {1, 1}))
    NodeDef* pack = get_mutable_node(node->input(2));
    CHECK_NODE_OK(IsPack(*pack) && pack->input_size() == 2)
    CHECK_NODE_OK(check_const_value<int>(get_mutable_node(pack->input(0)), {0}))
    NodeDef* minimum = get_mutable_node(pack->input(1));
    int minimum_i = 0;
    NodeDef* minimum_v = get_mutable_node(minimum->input(0));
    if (IsConstant(*minimum_v) || IsHostConstant(*minimum_v)) minimum_i = 1;
    CHECK_NODE_OK(minimum->op() == "Minimum" && minimum->input_size() == 2)
    NodeDef* strided_slice = get_mutable_node(minimum->input(minimum_i));
    CHECK_NODE_OK(strided_slice->op() == "StridedSlice" &&
                  strided_slice->input_size() == 4)
    CHECK_NODE_OK(check_int_attr(strided_slice, "shrink_axis_mask", 1))
    CHECK_NODE_OK(
        check_const_value<int>(get_mutable_node(strided_slice->input(1)), {-1}))
    CHECK_NODE_OK(
        check_const_value<int>(get_mutable_node(strided_slice->input(2)), {0}))
    CHECK_NODE_OK(
        check_const_value<int>(get_mutable_node(strided_slice->input(3)), {1}))
    NodeDef* shape = get_mutable_node(strided_slice->input(0));
    CHECK_NODE_OK(IsShape(*shape) && shape->input_size() == 1)
    CHECK_NODE_OK(get_node(shape->input(0)) == get_node(node->input(0)))
    CHECK_NODE_OK(HasNodeAttr(*node, "T"))
    ...
    fused_node->set_device(node->device());
    fused_node->add_input(shape->input(0));
    fused_node->add_input(minimum->input(1 - minimum_i));
    AddNodeAttr("T", node->attr().at("T"), fused_node);
    nodes->SwapElements(node_indexes.at(node->name()), nodes->size() - 1);
...
void run_graph_optimization(GraphDef* graph) {
  ...
  if (enabled_aarch64_rewriters()) {
    ...
    if (enable_all || FLAGS_annc_fused_direct_hash_mod)
	    optimizer.register_rewriter(
			    std::make_unique<KPFusedDirectHashModRewriter>());
    if (enable_all || FLAGS_annc_fused_dynamic_padding)
	    optimizer.register_rewriter(
			    std::make_unique<KPFusedDynamicPaddingRewriter>());
    if (enable_all || FLAGS_annc_fused_trunc_seq)
	    optimizer.register_rewriter(
			    std::make_unique<KPFusedTruncSeqRewriter>());
