...
DECLARE_bool(enable_switch_opt);
...
std::unique_ptr<GraphOptimizer> MetaOptimizer::MakeNewOptimizer(
  ...
  MK_OPT("switch", "switch_optimization", new SwitchOptimizer());
...
#define BOTH_ARE_EXPERIMENTAL_BOTH(CFG) \
  ...
  if (BOTH_NOT_OFF(switch_optimization)) {
    if (USER_IS_EXPERIMENTAL_MLIR(switch_optimization) ||
        USER_IS_EXPERIMENTAL_BOTH(switch_optimization))
      VLOG(2) << "switch_optimization is not implemented in TFG yet";
    else
      optimizers->push_back(std::make_unique<SwitchOptimizer>());
  }
...
#define PRINT_CFG(NAME, CONFIG) \
  if (optimizer_name == NAME)   \
    user_cfg.toggle_config[CONFIG] = RewriterConfig::ON;
      ...
      PRINT_CFG("switch", "switch_optimization")
...
bool MetaOptimizerEnabled(const ConfigProto& cfg) {
  ...
  return !rewrite_cfg.disable_model_pruning() ||
         ...
         rewrite_cfg.switch_optimization() != RewriterConfig::OFF ||
