  ...
  class KernelStats {
   public:
    ...
    void UpdateCostEstimate(const NodeItem& node, uint64 elapsed_cycles) {
      ...
      uint64 new_estimate =
          ((kCostDecay - 1) * prev_estimate + kAArch64Scale * elapsed_cycles) / kCostDecay;
#else
      uint64 new_estimate =
...
#if defined(__aarch64__)
//kunpeng 7282C main frequency is 2.9GHz, while the sample timer defaults to 100 MHz, requiring a scaling factor.
    static constexpr uint64 kAArch64Scale = 29;
#endif    
