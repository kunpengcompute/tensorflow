...
DEFINE_bool(trace_timing, false, "Log server-side per-request timing breakdown");
DEFINE_int32(trace_timing_max_requests, 100, "Maximum server-side timing logs to print");
...
DEFINE_bool(tf_timeline_log_each_dump, true, "Log each RunMetadata dump result");
...
public:
    ...
    virtual void Echo(google::protobuf::RpcController* cntl_base,
        ...
        bool ret = tf_model.Infer(request->message());
        std::string echo_str = request->message();
        if (ret) {
            echo_str += ", tf model infer success!";
        } else {
            echo_str += ", tf model infer fail!";
        }

        // Fill response.
        response->set_message(echo_str);
