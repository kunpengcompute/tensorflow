...
- **数据并行**：按输出矩阵的列维度进行分片，每个线程处理独立的列块。
- **无锁设计**：为每个线程分配独立缓冲区，避免线程间同步开销，同时消除伪共享问题。
- **负载均衡**：采用均匀分片策略，使各线程工作量尽可能均衡。
- **内存优化**：使用内存对齐分配接口，提升SIMD向量化执行效率。
...
- **列分片策略**：将输出矩阵的列按线程数均匀分割。
- **线程私有缓冲区**：为每个线程分配独立的计算缓冲区，避免数据竞争。
- **并行执行**：复用框架线程池，KDNN将计算任务提交到框架线程池统一调度。
- **单线程回退条件**：在线程池指针为空、当前已在并行区域或线程池线程数小于等于1时使用串行算法。
...
ANNC静态图融合特性开关通过代码补丁的方式接入TensorFlow，基于TensorFlow v2.15版本增加。
...
TensorFlow Serving线程调度优化特性以Patch的方式实现，并合入了openEuler组织的sra\_tensorflow\_adapter开源仓库，基于TF Serving/TensorFlow v2.15版本新增以下两种特性开关。
...
首先介绍TensorFlow Serving推理时使用的线程池，以更好理解本特性的工作原理，从而根据实际场景决定特性的开关和设置。

**图 12** TensorFlow Serving线程池运行视图<a name="fig158087456394"></a><a id="TF Serving线程池运行视图"></a>

![TensorFlow Serving线程池运行视图](figures/TF-Serving线程池运行视图.png "TF-Serving线程池运行视图")

TensorFlow Serving用于推理的线程大致分为两类：通信线程和计算线程。
...
改进后，减少了Session间推理任务的互相干扰，使得单个Session能够以更低的时延完成推理，并增强了TensorFlow Serving的并发性能。同时注意到通信线程和计算线程处理的是不同类型的任务，可以设置线程亲和性进行隔离，也能获得一定的性能收益。
...
| 文档版本 | 发布日期 | 修改说明 |
| ---- | ---- | --- |
| 03 | 2026-09-30 | 第三次正式发布。 <ul><li>新增KDNN SparseMatmul多线程优化特性介绍内容。</li><li>新增kembedding算子库EmbeddingTableLookup算子特性介绍内容。</li></ul> |
| 02 | 2026-06-30 | 第二次正式发布。 <ul><li>TensorFlow ANNC图编译优化特性增加常量折叠优化特性介绍内容。</li><li>新增TensorFlow ANNC静态图融合特性，增加对应特性介绍，软件架构等内容。</li></ul> |
| 01 | 2026-03-30 | 第一次正式发布。 <ul><li>新增TensorFlow KDNN线程直通特性，增加对应特性介绍，软件架构等内容。</li></ul> |
